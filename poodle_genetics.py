...
_ALLELE_COLOR = {
    ...
    # D座位 (MLPH) — 鮮やか青系
    "D":  "#1E40AF",  "d":  "#93C5FD",
    # K座位 (CBD103) — KB=ドミナントブラック（黒）、ky=ファントム タン（シエナ）
    "KB": "#0a0a0a",  "K":  "#0a0a0a",  "ky": "#A0522D",  "kbr": "#A16207",
    # M座位 (PMEL) — ターコイズ系
    "m":  "#0E7490",  "M":  "#22D3EE",
    # A座位 (ASIP) — 暖色タン系
...
# ============================================================
# Embark DNA テスト由来の追加疾患（embark_diseases.py）
# ============================================================
# DISEASE_KB の末尾に追記することで、既存スキーマ（slug 索引・カテゴリ分類・
# 症状フィルタ・関連リンク等）にそのまま乗る。ロードに失敗してもアプリは動作。
# 各エントリには `_source="embark"` フラグを付与し、初心者向け解説（_simple）
# が未投入であることを許容する（コンテンツチームが順次追記する）。
try:
    from embark_diseases import EMBARK_NEW_DISEASES
    for _e in EMBARK_NEW_DISEASES:
        _e.setdefault("_source", "embark")
    DISEASE_KB.extend(EMBARK_NEW_DISEASES)
    HAS_EMBARK_DISEASES = True
except ImportError:
    HAS_EMBARK_DISEASES = False

# 品種別バリアント（Embark の breed-specific variant 検査ごとに個別エントリ化）
try:
    from embark_diseases_variants import EMBARK_VARIANT_DISEASES
    for _e in EMBARK_VARIANT_DISEASES:
        _e.setdefault("_source", "embark")
    DISEASE_KB.extend(EMBARK_VARIANT_DISEASES)
    HAS_EMBARK_VARIANTS = True
except ImportError:
    HAS_EMBARK_VARIANTS = False
...
# 辞書ページでグルーピング表示に使用
DISEASE_CATEGORIES = [
    ("🦴 骨格・関節系",   ["椎間板", "骨軟骨", "短足", "骨形成不全", "軟骨異形成症"]),
    ("🧠 神経・脳系",     ["脳症", "リポフスチン", "運動失調", "脊髄小脳", "多剤耐性", "ガングリオシドーシス", "変性性脊髄症", "筋強直症", "小脳皮質", "感覚性神経", "ジストロフィー", "白質ジストロフィー", "Krabbe", "多発性神経", "発作性失神", "ヒドロキシグルタル", "髄膜脳炎", "ラフォラ", "ナルコレプシー", "神経軸索", "海綿状白質", "てんかん", "アレキサンダー病", "フコシドーシス", "マンノシドーシス", "ラゴット蓄積症", "発作性運動異常症", "歯・骨格・網膜", "小脳失調", "小脳低形成", "髄鞘形成不全", "脱髄性多発神経炎", "喉頭麻痺", "新生子小脳", "シェイキングパピー", "ハスキー脳症", "末端自咬", "ピルビン酸脱水素", "アシル CoA", "コハク酸セミアルデヒド", "多系統変性", "繊毛運動不全", "木質性膜炎", "小脳変性"]),
    ("👁 眼科系",         ["緑内障", "夜盲", "コリーアイ", "白内障", "全色盲", "錐体杆体", "進行性網膜萎縮", "PRA", "多巣性網膜症", "CMR", "錐体ジストロフィー", "スターガルト", "rcd", "水晶体脱臼", "角膜", "ドライアイ", "小眼球", "網膜異形成", "難聴・前庭", "難聴 + 前庭", "両側性難聴", "成犬発症難聴", "片側性難聴", "昼盲症"]),
    ("💜 心臓系",         ["心筋症", "不整脈", "QT 延長", "若年突然死"]),
    ("🩸 血液・凝固系",   ["フォン・ヴィレブランド", "ピルビン酸", "第VII", "第 VII", "第 XI", "プレカリクレイン", "血小板", "メトヘモグロビン", "フルクトキナーゼ", "血友病", "グランツマン", "ベルナール", "楕円赤血球", "メイ・ヘグリン", "赤血球異形成", "白血球接着不全", "補体", "スコット症候群"]),
    ("🧪 代謝・内分泌系", ["尿酸尿", "コバラミン", "グリコーゲン蓄積", "ムコ多糖", "銅蓄積", "高リン血症", "リン血症", "ポンペ病", "高シュウ酸尿", "尿酸結石", "キサンチン", "ジヒドロキシアデニン", "甲状腺機能", "ビタミン D", "アルカリホスファターゼ", "炎症性ミオパチー", "ALT 活性", "腸管脂質吸収不良", "脂質吸収", "無汗性外胚葉", "中鎖アシル", "低カタラーゼ", "糖原病"]),
    ("💪 筋・運動系",     ["運動誘発性", "中心核ミオパチー", "肢帯型筋", "重症筋無力症", "骨格異形成", "筋ジストロフィー", "ネマリン", "ミオパチー", "マスラディン", "胎児発症型", "新生子神経軸索", "ブリーウィペット", "悪性高熱症", "ジストログリカン", "マラミュート多発神経炎", "筋無力症候群"]),
    ("🫘 腎・泌尿器系",   ["シスチン尿", "腎症", "腎嚢腺癌", "蛋白漏出性腎症", "ファンコニ", "嚢胞腎", "嚢胞性腎"]),
    ("🧴 皮膚・被毛系",   ["鼻過角化", "魚鱗癬", "希釈性脱毛", "肉球角化", "白皮症", "角化異常", "肢端皮膚炎", "表皮水疱症", "エーラス・ダンロス", "ダリエ病", "爪甲肥厚", "皮膚脆弱", "エナメル質形成不全", "腿無毛", "ドライアイ・カーリーコート"]),
    ("🛡 免疫系",         ["好中球", "重症複合免疫不全", "再発性炎症性肺", "繰り返す炎症性肺", "ルンデフント症候群", "自己炎症性", "補体 C3"]),
    ("🌱 発達・内分泌系", ["下垂体性小人症", "比例性小人症", "不均衡性小人症", "ミュラー管", "口唇口蓋裂", "口蓋裂", "頭蓋下顎骨症", "新生子間質性肺", "レイン症候群"]),
    ("🫃 消化器系",       ["消化管"]),
]
