...
- **🆕 遺伝子疾患・形質辞書 (`/glossary`)** — 77疾患 + 27形質座位の詳細解説。カテゴリ別グルーピング、重症度フィルター（🔴高/🟡中/🟢低）、症状ベース検索（後肢麻痺・視覚障害・出血傾向等）、全文検索
...
- **毛色シミュレーション** — メンデル遺伝に基づく子犬の毛色予測（G座位対応でシルバー/カフェオレ（シルバービーグ）自動算出）
...
- `/glossary?lang=en` — 全 77 疾患 + 27 形質座位の英訳表示
...
解析結果がなくても単独で利用可能な **77疾患 + 27形質座位** の総合辞書。
